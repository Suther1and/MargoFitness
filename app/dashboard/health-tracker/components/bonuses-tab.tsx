            ...
            <p className="mt-1 text-white/40 text-xs leading-relaxed">
              {(() => {
                ...
                if (!advancedMentor?.isUnlocked) {
                  ...
                  const friendWord = remaining === 1 ? 'друга' : 'друзей'
                  const pronounWord = remaining === 1 ? 'его' : 'их'
                  const purchaseWord = remaining === 1 ? 'покупки' : 'покупок'
                  
                  return (
                    <>Пригласи еще <span className="font-bold text-white/70">{remaining}</span> {friendWord}, ты получишь <span className="font-bold text-white/70">750</span> <Sparkles className="w-3 h-3 text-current inline" /> после {pronounWord} первой {purchaseWord}. Ты получаешь <span className="font-bold text-white/70">{percent}%</span> <Sparkles className="w-3 h-3 text-current inline" /> от всех покупок своих друзей.</>
                  )
                }
                ...
                if (!guru?.isUnlocked) {
                  ...
                  return (
                    <>Пригласи еще <span className="font-bold text-white/70">{remaining}</span> {friendWord}, ты получишь <span className="font-bold text-white/70">1000</span> <Sparkles className="w-3 h-3 text-current inline" /> после {pronounWord} первой {purchaseWord}. Ты получаешь <span className="font-bold text-white/70">{percent}%</span> <Sparkles className="w-3 h-3 text-current inline" /> от всех покупок своих друзей.</>
                  )
                }
