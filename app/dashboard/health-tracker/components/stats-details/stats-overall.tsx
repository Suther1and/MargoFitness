...
export function StatsOverall({ period, onNavigate, layout = 'column', data }: StatsOverallProps) {
  ...
  const renderDesktopLayout = () => (
    ...
    >
      {/* 1. Блок Трансформация (Вес) - Горизонтальный и строгий (2x1) */}
      {settings.widgets.weight?.enabled && (
        <motion.div
          variants={item}
          onClick={() => onNavigate?.('weight')}
          className="col-span-2 bg-[#121214]/60 border border-white/10 rounded-[1.5rem] p-6 hover:border-emerald-500/20 transition-all group relative overflow-hidden flex flex-col justify-between"
        >
          <div className="flex justify-between items-start relative z-10">
            <div>
              <div className="text-[10px] font-black text-emerald-400 uppercase tracking-[0.3em] mb-1">Анализ веса</div>
              <h3 className="text-2xl font-black text-white uppercase tracking-tight">Трансформация</h3>
            </div>
            <div className="flex flex-col items-end">
              <div className="text-3xl font-black text-emerald-400 tabular-nums leading-none">
                -1.8 <span className="text-xs font-black opacity-40">КГ</span>
              </div>
              <div className="text-[9px] font-black text-white/20 uppercase tracking-widest mt-1">за 7 дней</div>
            </div>
          </div>

          <div className="absolute inset-x-0 bottom-0 h-24 opacity-10 pointer-events-none">
            <svg className="w-full h-full" preserveAspectRatio="none" viewBox="0 0 100 40">
              <path d="M0 35 Q 25 30, 50 20 T 100 5" fill="none" stroke="white" strokeWidth="1" />
            </svg>
          </div>

          <div className="flex items-center gap-12 mt-8 relative z-10 border-t border-white/5 pt-6">
            <div className="flex-1">
              <div className="text-[10px] font-black text-white/20 uppercase tracking-widest mb-1.5">Начало периода</div>
              <div className="flex items-baseline gap-1">
                <span className="text-2xl font-black text-white/90 tabular-nums">74.2</span>
                <span className="text-[10px] font-black text-white/20 uppercase">кг</span>
              </div>
            </div>
            <div className="w-px h-8 bg-white/5" />
            <div className="flex-1">
              <div className="text-[10px] font-black text-white/20 uppercase tracking-widest mb-1.5">Текущий вес</div>
              <div className="flex items-baseline gap-1">
                <span className="text-2xl font-black text-white tabular-nums">72.4</span>
                <span className="text-[10px] font-black text-white/20 uppercase">кг</span>
              </div>
            </div>
            <div className="flex-1 text-right">
              <div className="text-[10px] font-black text-white/20 uppercase tracking-widest mb-1.5">ИМТ</div>
              <div className="text-xl font-black text-white/60 tabular-nums">{bmiValue}</div>
            </div>
          </div>
        </motion.div>
      )}

      {/* 2. Дисциплина (1x1) - Тонкое и профессиональное кольцо */}
      {settings.widgets.habits?.enabled && (
        <motion.div
          variants={item}
          onClick={() => onNavigate?.('habits')}
          className="col-span-1 bg-[#121214]/60 border border-white/10 rounded-[1.5rem] p-6 hover:border-amber-500/20 transition-all group flex flex-col items-center justify-center text-center"
        >
          <div className="absolute top-5 left-6">
            <div className="text-[10px] font-black text-white/20 uppercase tracking-[0.3em]">Стабильность</div>
          </div>
          ...
          <div className="relative w-28 h-32 flex items-center justify-center">
            <svg className="w-24 h-24 -rotate-90" viewBox="0 0 100 100">
              ...
              <motion.circle 
                ...
                animate={{ strokeDashoffset: 289 * (1 - 0.75) }}
                className="text-amber-500" 
                strokeLinecap="round" 
              />
            </svg>
            <div className="absolute inset-0 flex items-center justify-center">
              <span className="text-2xl font-black text-white tabular-nums">75%</span>
            </div>
          </div>

          <div className="mt-2 flex items-center gap-2">
            <div className="w-1.5 h-1.5 rounded-full bg-amber-500 animate-pulse" />
            <span className="text-lg font-black text-white tabular-nums">12</span>
            <span className="text-[9px] font-black text-white/30 uppercase tracking-widest">дней стрик</span>
          </div>
        </motion.div>
      )}

      {/* 3. Питание (1x1) - Баланс и чистота */}
      {settings.widgets.nutrition?.enabled && (
        <motion.div
          variants={item}
          onClick={() => onNavigate?.('nutrition')}
          className="col-span-1 bg-[#121214]/60 border border-white/10 rounded-[1.5rem] p-6 hover:border-violet-500/20 transition-all group flex flex-col justify-between"
        >
          <div>
            <div className="text-[10px] font-black text-violet-400 uppercase tracking-[0.3em] mb-1">Калории</div>
            <h3 className="text-xl font-black text-white/40 uppercase tracking-tight">Среднее</h3>
          </div>

          <div className="my-4">
            <div className="flex items-baseline gap-1.5">
              <span className="text-4xl font-black text-white tabular-nums tracking-tighter">2,050</span>
              <span className="text-[10px] font-black text-white/20 uppercase tracking-widest">ккал</span>
            </div>
          </div>

          <div className="pt-4 border-t border-white/5">
            <div className="flex justify-between items-end mb-2">
              <span className="text-[9px] font-black text-white/20 uppercase tracking-widest">Цель {settings.widgets.nutrition?.goal || 2200}</span>
              <span className="text-[10px] font-black text-emerald-400 uppercase tracking-widest">В норме</span>
            </div>
      ...
      {/* 4. Прогресс Формы (2x1) */}
      {settings.widgets.photos?.enabled && (
        <motion.div
          variants={item}
          onClick={() => onNavigate?.('photos')}
          className="col-span-2 bg-[#121214]/60 border border-white/10 rounded-[1.5rem] p-6 hover:border-violet-500/20 transition-all group"
        >
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3">
              ...
              <h4 className="text-lg font-black text-white uppercase tracking-tight">Прогресс формы</h4>
            </div>
            <span className="text-[9px] font-black text-white/20 uppercase tracking-widest">за 7 дней</span>
          </div>

          <div className="grid grid-cols-2 gap-4 h-[180px]">
            <div className="relative rounded-2xl overflow-hidden bg-white/5 border border-white/5 group/photo flex items-center justify-center">
              ...
              <div className="absolute top-3 left-3 px-2 py-0.5 rounded bg-black/40 backdrop-blur-md text-[8px] font-black uppercase text-white/40 tracking-widest">До</div>
              <div className="absolute bottom-3 left-3 flex justify-between items-end w-[calc(100%-24px)]">
                <span className="text-[9px] font-bold text-white/20 tabular-nums">31 дек</span>
                <span className="text-[11px] font-black text-white/30 tabular-nums">74.2 кг</span>
              </div>
            </div>
            <div className="relative rounded-2xl overflow-hidden bg-white/5 border border-white/5 group/photo flex items-center justify-center">
              <div className="absolute inset-0 flex flex-col items-center justify-center opacity-30">
                <Camera className="w-10 h-10 text-violet-500/20 mb-2" />
                <span className="text-[8px] font-black uppercase tracking-widest text-violet-500/40">Ожидание</span>
              </div>
              <div className="absolute top-3 right-3 px-2 py-0.5 rounded bg-emerald-500/20 backdrop-blur-md border border-emerald-500/20 text-[8px] font-black uppercase text-emerald-400 tracking-widest">После</div>
              <div className="absolute bottom-3 left-3 flex justify-between items-end w-[calc(100%-24px)]">
                <span className="text-[9px] font-bold text-white/20 tabular-nums">7 янв</span>
                <span className="text-[11px] font-black text-white/30 tabular-nums">72.4 кг</span>
              </div>
            </div>
          </div>
        </motion.div>
      ...
      {/* 5. Шаги и Вода (1x1 каждый) - Квадратные и сбалансированные */}
      {settings.widgets.steps?.enabled && (
        <motion.div
          variants={item}
          onClick={() => onNavigate?.('steps')}
          className="col-span-1 bg-[#121214]/60 border border-white/10 rounded-[1.5rem] p-6 hover:border-red-500/20 transition-all group flex flex-col justify-between"
        >
          <div>
            <div className="text-[10px] font-black text-red-500 uppercase tracking-[0.3em] mb-1">Активность</div>
            <h3 className="text-xl font-black text-white/40 uppercase tracking-tight">Шаги</h3>
          </div>

          <div className="my-2">
            <span className="text-4xl font-black text-white tabular-nums tracking-tighter">11,200</span>
          </div>

          <div className="pt-4 border-t border-white/5">
            <div className="flex justify-between items-end mb-2">
              <span className="text-[9px] font-black text-white/20 uppercase tracking-widest">Цель 10к</span>
              <span className="text-[10px] font-black text-red-500 tabular-nums">5 / 7</span>
            </div>
            <div className="h-1 bg-white/5 rounded-full overflow-hidden">
      ...
      {settings.widgets.water?.enabled && (
        <motion.div
          variants={item}
          onClick={() => onNavigate?.('water')}
          className="col-span-1 bg-[#121214]/60 border border-white/10 rounded-[1.5rem] p-6 hover:border-blue-500/20 transition-all group flex flex-col justify-between"
        >
          <div>
            <div className="text-[10px] font-black text-blue-400 uppercase tracking-[0.3em] mb-1">Гидрация</div>
            <h3 className="text-xl font-black text-white/40 uppercase tracking-tight">Вода</h3>
          </div>

          <div className="my-2">
            <div className="flex items-baseline gap-1">
              <span className="text-4xl font-black text-white tabular-nums tracking-tighter">2.3</span>
              <span className="text-[10px] font-black text-white/20 uppercase tracking-widest">л</span>
            </div>
          </div>

          <div className="pt-4 border-t border-white/5">
            <div className="flex justify-between items-end mb-2">
              <span className="text-[9px] font-black text-white/20 uppercase tracking-widest">Стабильность</span>
              <span className="text-[10px] font-black text-blue-400 tabular-nums">85%</span>
            </div>
            <div className="h-1 bg-white/5 rounded-full overflow-hidden">
      ...
      {/* 6. Нижний ряд: Режим (Сон/Кофе) и Заметки (4x1) */}
      <div className="col-span-4 grid grid-cols-4 gap-6">
        <div className="col-span-1 grid grid-rows-2 gap-4">
          <div onClick={() => onNavigate?.('sleep')} className="bg-[#121214]/40 border border-white/5 rounded-2xl p-4 flex items-center justify-between hover:bg-white/[0.03] transition-all group cursor-pointer">
            <span className="text-[10px] font-black text-white/20 uppercase tracking-widest">Сон</span>
            <span className="text-sm font-black text-indigo-400 tabular-nums">7.6ч</span>
          </div>
          <div onClick={() => onNavigate?.('caffeine')} className="bg-[#121214]/40 border border-white/5 rounded-2xl p-4 flex items-center justify-between hover:bg-white/[0.03] transition-all group cursor-pointer">
            <span className="text-[10px] font-black text-white/20 uppercase tracking-widest">Кофе</span>
            <span className="text-sm font-black text-amber-600 tabular-nums">2.1</span>
          </div>
        </div>

        <div onClick={() => onNavigate?.('mood')} className="col-span-1 bg-[#121214]/40 border border-white/5 rounded-[1.5rem] p-5 flex flex-col items-center justify-center hover:bg-white/[0.03] transition-all cursor-pointer group">
          <Smile className="w-6 h-6 text-pink-400/40 group-hover:text-pink-400 transition-colors mb-2" />
          <span className="text-xs font-black text-white/60 uppercase tracking-tight">Отличное</span>
        </div>

        {/* Заметки (2 колонки) */}
        <div onClick={() => onNavigate?.('notes')} className="col-span-2 bg-[#121214]/60 border border-white/10 rounded-[1.5rem] p-6 hover:border-sky-500/20 transition-all group cursor-pointer relative overflow-hidden">
          <div className="flex items-center justify-between mb-4">
            <div className="text-[10px] font-black text-white/20 uppercase tracking-[0.3em]">Последние мысли</div>
            <ChevronRight className="w-4 h-4 text-white/10 group-hover:text-sky-400 transition-colors" />
          </div>
          <p className="text-[11px] text-white/40 leading-relaxed italic line-clamp-2 border-l-2 border-sky-500/20 pl-4">
            "{MOCK_LAST_NOTE.content}"
          </p>
          <div className="absolute bottom-4 right-6 text-[8px] font-black text-white/5 uppercase tracking-widest">12 записей за неделю</div>
        </div>
      </div>
    </motion.div>
  )
