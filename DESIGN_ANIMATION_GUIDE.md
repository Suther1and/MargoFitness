...
### ✅ MutationObserver для гарантированного запуска CSS анимаций на мобильных

**Проблема:** CSS анимации модальных окон могут не срабатывать на мобильных из-за race condition с библиотеками (например, Radix UI).

**Решение:** Используй MutationObserver для отслеживания появления элемента в DOM и принудительного перезапуска анимации.
...
useEffect(() => {
  ...
  if (isMobile) {
    // Для мобильных: агрессивный подход с MutationObserver
    ...
    // Для десктопа: простой force reflow обычно достаточно
...
### ✅ Разные CSS анимации для desktop и mobile

**Для лучшего UX используй разные параметры анимаций:**

```css
/* Анимация для desktop */
...
/* Анимация для mobile - более драматичная */
@keyframes popupScaleInMobile {
  from {
    opacity: 0;
    transform: scale(0.5); /* Начинаем с меньшего размера */
  }
  to {
    opacity: 1;
    transform: scale(1);
...
### 📌 Ключевые моменты для CSS анимаций модальных окон

1. **MutationObserver** обязателен для мобильных если анимация не срабатывает
2. **animation-fill-mode: both** гарантирует применение стилей до и после анимации
3. **Разные scale значения**: desktop (0.9), mobile (0.5-0.7) - на мобильных нужна более заметная анимация
4. **Разная длительность**: mobile обычно дольше (0.6-0.8s), desktop быстрее (0.3-0.5s)
5. **Force reflow** через `void element.offsetHeight` перед перезапуском анимации
6. **Таймаут для cleanup** MutationObserver через 2 секунды
...
- [ ] **Модальные окна появляются с плавной анимацией** (не "прыгают")
- [ ] Анимации модальных окон **заметные и плавные** (не слишком быстрые)
...
- [ ] Модальные окна появляются с bounce эффектом (если задано)
...
6. **CSS анимации модальных окон не работают на мобильных**
   - ❌ Используем только CSS анимации без JS поддержки
   - ❌ Одинаковые параметры анимаций для desktop и mobile
   - ❌ Слишком быстрые/незаметные анимации на мобильных (< 0.4s, scale от 0.9)
   - ✅ MutationObserver для отслеживания появления элемента в DOM
   - ✅ Force reflow через `void element.offsetHeight`
   - ✅ `animation-fill-mode: both` для гарантии применения стилей
   - ✅ Отдельные keyframes для mobile (scale от 0.5-0.7, длительность 0.6-0.8s)
   - ✅ Более медленные и заметные анимации на мобильных устройствах
...
**Версия:** 1.1  
**Основано на:** `app/design-test/dashboard/page.tsx`, `components/signin-popup.tsx`
